...
export class StuckDealsPerOwnerRPT {
    ...
    async init(c: any): Promise<void> {
        ...
        this.url = this.hsSettings.executiveReports.stuckDeals.stuckDealsPerOwnerDataRetrieveInternalLink.url;
        this.method = this.hsSettings.executiveReports.stuckDeals.stuckDealsPerOwnerDataRetrieveInternalLink.method;
        ...
        this.rptTitle = this.hsSettings.executiveReports.stuckDeals.stuckDealsPerOwnerRptMeta.rptTitle;
        this.rptSubject = this.hsSettings.executiveReports.stuckDeals.stuckDealsPerOwnerRptMeta.rptSubject;
        this.rptComments = this.hsSettings.executiveReports.stuckDeals.stuckDealsPerOwnerRptMeta.rptComments;
        this.rptKeywords = this.hsSettings.executiveReports.stuckDeals.stuckDealsPerOwnerRptMeta.rptKeywords;
        ...
        await this.createSalesTeamMessages(this.rptData);
    ...
    private async createExecMessages(execUsers: any[], execMessageParameters: any) {
        let execMessageTemplate = this.hsSettings.executiveReports.stuckDeals.execReportsMessageTemplate_stuckPerOwner;
    ...
    private createExecReportBlockMessageParameters(data: any) {
        ...
        owners.forEach((owner: any) => {
            ...
            const ownerCount = owner.stuck_deals_count;
            const ownerItem = sprintf(messageDetailsItem, `mailto: ${ownerEmail}`, ownerName, ownerCount);
            
            const ownerField = this.interpolateObject(
                this.hsSettings.executiveReports.stuckDeals.stuckDealsPerStageRptStageBlockTemplate_stuckPerOwner,
                {ownerDetails: ownerItem}
            );
            ownerFields.fields.push(ownerField);
        })
    ...
    private async createHsAdminMessages(hsAdminsUsers: any[], hsAdminsMessageParameters: any) {
        let hsAdminsReportsMessageTemplate = this.hsSettings.executiveReports.stuckDeals.hsAdminsReportsMessageTemplate_stuckPerOwner;
    ...
    private async createSalesTeamMessages(data: any) {
        let salesTeamReportsMessageTemplate = this.hsSettings.executiveReports.stuckDeals.salesTeamReportsMessageTemplate_stuckPerOwner;
        const owners = data.owners;

        for (const owner of owners) {
            const messageParams = { ownerStuckDealsCount: owner.stuck_deals_count };
            const xlsSalesRpt = this.createSalesMessagesXLS(owner.stuck_deals);
            const ownersList = [owner.owner_email]; // needs to be array because getSlackUsersByEmails wants array as arg
            try {
                const slackUser = await this.getSlackUsersByEmails(ownersList, this.slackToken);
                this.addToSlackDMList(slackUser, salesTeamReportsMessageTemplate, messageParams, xlsSalesRpt);
            } catch (error) {
                console.error(`Error processing owner ${owner.owner_email}:`, error);
            }
        }

    }    
    
    private async createSalesMessagesXLS(data: any): Promise<Uint8Array> {
        ...
        /* DEALS DETAILS worksheet */
        {
            ...
            let ownerDetails: ownerDealsItem[] = [];
            ...
            deals.forEach((deal: any) => {
                ...
                const dealLastModified = this.formatDateToString(new Date(deal.lastModifiedDate));
            ...
            let customHeaders = [['Deal', 'Stage', 'Last modified']];
            ...
            ownerDetails.forEach((item, i) => {
                const r = startRow + i;

                // dealName in column B (c = 1)
                ...
                // dealStage in column C (c = 2)
                const dealStageCell = XLSX.utils.encode_cell({ r, c: startCol + 2 });
                ws_Details[dealStageCell] = {
                    t: 's',
                    v: item.dealStage,
                    //l: { Target: item.dealLink }
                };
                ...
                const lastModifiedCell = XLSX.utils.encode_cell({ r, c: startCol + 3 });
            ...
            range.e.r = Math.max(range.e.r, startRow + ownerDetails.length - 1);
        ...
        const styledBuffer = await this.styleSheetJSSalesBuffer(xlsxArrayBuffer)
        return styledBuffer;
        //return xlsxArrayBuffer; // skip styling to check data only
    }

    private async styleSheetJSSalesBuffer(sheetJSBuffer: Uint8Array): Promise<Uint8Array>  {
        ...
        const ws_Details = workbook.sheet(0);
    ...
    private async createReportAdminMessages(reportAdminsUsersUsers: any[]) {
        let rptAdminsReportsMessageTemplate = this.hsSettings.executiveReports.stuckDeals.reportAdminsReportsMessageTemplate_stuckPerOwner;
    ...
    private async createExecMessagesXLS(data: any): Promise<Uint8Array> {
        ...
        /* SUMMARY worksheet */
        {
            ...
            // owners summary           
            let ownersSummary: preambleItem[] = [];
            owners.forEach( (owner: any) => {
                ownersSummary.push({
                    info: owner.owner_name,
                    infoValue: owner.stuck_deals_count,
                });
            });
            ...
            XLSX.utils.sheet_add_aoa(ws_Summary, [['Owner', 'Count']], { origin: `B6` });

            // Add owner data with hyperlinks row-by-row (because of links)
            owners.forEach((owner: any, i: number) => {
                const ownerLink = `mailto: ${owner.owner_email}`;
                ...
                ws_Summary[cellRef] = {
                    t: 's', // type: string
                    v: owner.owner_name, // display value
                    l: { Target: ownerLink } // hyperlink
                };
            ...
            range.e.r = Math.max(range.e.r, 6 + owners.length);
        ...
        /* DEALS DETAILS worksheet */
        {
            ...
            // create sheet and headers
            let customHeaders = [['Owner', 'Deal', 'Stage', 'Last modified']];

            // set autofilter 
            ws_Details['!autofilter'] = { ref: "B2:E2" };
            ...
            ownerDetails.forEach((item, i) => {
                ...
                const ownerNameCell = XLSX.utils.encode_cell({ r, c: startCol });
                ws_Details[ownerNameCell] = {
                    t: 's',
                    v: item.ownerName,
                    l: { Target: item.ownerLink }
                };
                ...
                // dealLastModified in column E (c = 4)
                const lastModifiedCell = XLSX.utils.encode_cell({ r, c: startCol + 3 });
                ws_Details[lastModifiedCell] = {
                    t: 's',
                    v: item.dealLastModified
        ...
        //Apply styles to the second sheet (ws_Details)
        {
            ...
            // From B2 downward → Apply plain borders while content exists
            {
                ...
                while (true) {
                    ...
                    ws_Details.range(`B${row}:E${row}`).style("border", true);
                    row++;
                }
            }

            // B2:E2 → Bold, yellow background
            ws_Details.range("B2:E2").style({
                bold: true,
                fill: "ffff00"
            });
